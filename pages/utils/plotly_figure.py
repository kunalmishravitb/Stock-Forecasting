...
import dateutil
import pandas_ta as pta # pandas_ta means pandas time series analysis
import datetime

# Plotly table function
def plotly_table(dateframe):
    headerColor = 'grey'
    rowEvenColor = '#f8fafd'
    rowOddColor = '#e1efff'
    ...
    fig = go.Figure(data=[go.Table(
        header = dict(
            values=["<b><b>"]+["<b>" + str(i)[:10]+"<b>" for i in dateframe.columns],
            line_color='#0078ff', fill_color='#0078ff',
            align='center', font=dict(color='white', size=15), height = 35,
        ),
        cells = dict(
            values=[["<b>"+str(i)+"<b>" for i in dateframe.index]]+[dateframe[i] for i in dateframe.columns], fill_color=[[rowOddColor, rowEvenColor]],
...
def filter_data(dataframe, num_period):
    if num_period == '1mo':
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-1) # iska matlab aaj ke din se 1 month minus
    elif num_period == '5d':
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(days=-5) # iska matlab aaj ke din se 5 days minus
    elif num_period == '6mo':
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(months=-6) # iska matlab aaj ke din se 6 months minus
    elif num_period == '1y':
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(years=-1) # iska matlab aaj ke din se 1 year minus
    elif num_period == '5y':
        date = dataframe.index[-1] + dateutil.relativedelta.relativedelta(years=-5) # iska matlab aaj ke din se 5 years minus
    elif num_period == 'ytd':
...
# Function for close chart
def close_chart(dataframe, num_period = False):
    if num_period:
        dataframe = filter_data(dataframe, num_period) # filtering data using number of period
    fig = go.Figure()
...
# Function to make candlestick chart
...
def Moving_average_forecast(forecast):
    ...
    fig.add_trace(go.Scatter(x=forecast.index[:-30], y=forecast['Close'].iloc[:-30], # Last 30 days ko chhod ke jitna data remaining hai vo show karenge
                            mode='lines',
                            name='Close Price', line=dict(width=2, color='black')))
    fig.add_trace(go.Scatter(x=forecast.index[-31:], y=forecast['Close'].iloc[-31:], # Last 30 days i.e. future ka 30 days ko show karenge
...
def Moving_average_forecast(forecast):
    ...
    fig.update_xaxes(title_text="Date", showgrid=True, tickformat="%b %d %Y")
    fig.update_yaxes(title_text="Stock Price", showgrid=True)
    ...
    legend=dict(
        yanchor="top",
        xanchor="right",
        bgcolor="white",  # Keep legend background white
        bordercolor="black",
        borderwidth=1,
        font=dict(color="black")  # Ensure legend text is readable
    ),
    xaxis=dict(title="Date", color="white"),  # X-axis label color
    yaxis=dict(title="Stock Price", color="white")  # Y-axis label color
    )
    return fig
